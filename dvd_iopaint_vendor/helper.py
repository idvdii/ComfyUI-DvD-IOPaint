...
import numpy as np
import torch
from dvd_iopaint_vendor.const import MPS_UNSUPPORT_MODELS
...
def get_cache_path_by_url(url):
    parts = urlparse(url)
    model_dir = os.environ.get("DVD_IOPAINT_MODEL_DIR")
    if not model_dir:
    ...
    if not os.path.exists(cached_file):
        ...
        if model_md5:
            ...
            if model_md5 == _md5:
                ...
                raise RuntimeError(
                    f"Model MD5 {_md5} does not match expected {model_md5}: "
                    f"{cached_file}"
                )
...
def get_image_ext(img_bytes):
    with Image.open(io.BytesIO(img_bytes)) as image:
        return (image.format or "JPEG").lower()
